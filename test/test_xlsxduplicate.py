...
def test_autoclean_get_list(get_test_file):
    ...
    assert clean_obj.get_columns_names() == ['First Name', 'Last Name', 'Gender', 'Age', 'Email', 'Phone', 'Occupation',
                                             'Custom column 1']
...
def test_clean(get_test_file):
    ...
    result = clean_obj.clean(out_filename=out_filename_str, order_column_name=None, ascending=True)
    assert result
...
def test_clean_with_ordering(get_autoclean_file):
    ...
    result = clean_obj.clean(out_filename=out_filename_str, order_column_name="Keep", ascending=True)
    assert result
    assert clean_obj.df.shape[0] < number_rows_before_cleaning
    # print(clean_obj.df)
